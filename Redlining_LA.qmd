...
redlining_map <- tm_shape(redlining) + # Map layer of redlined neighborhoods
  ...
  tm_polygons(fill = "grade", 
              palette = c("#6a994e", "#0077b6", "#ffb703", "#c1121f")) + # Fill in polygons based on HOLC grade
  ...
  #tm_graticules() +
  tm_scalebar(size = 0.5) + # Add scalebar
  tm_compass(position = c("left", "bottom"), size = 0.5) + # Add compass
...
#### CRS Matching 
In order to work with the EJ Screen and Redlining/HOLC dataset as one dataset, I need to first check if the coordinate reference system (CRS) match. 
```{r}
#| output: false
...
percent_grade_census <- ggplot(census_block_grade, aes(x = grade, y = percent_grade_HOLC, fill = grade)) +
  ...
  labs( x = "HOLC Grade", 
       ...
       title = "Census Block Groups within each HOLC Grade in LA County") +
  theme_minimal()
...
# Define columns
holc_grade <- c("A", "B", "C", "D", "NA")
percent_grade_HOLC <- sum_holc$percent_grade_HOLC
percent_not_HOLC <- sum_holc$percent_not_HOLC

# Combine into a table
holc_table <- data.frame(
  holc_grade,
  percent_grade_HOLC,
  percent_not_HOLC
)
...
holc_table %>%
  kable(col.names = c("HOLC grade", "% HOLC Grades in Census", "% HOLC Grades not in Census"), caption = "Table 1. Census Block Groups and HOLC Grades") %>%
